...
public class TileUtilities {
    ...
    /**
     * The {@link Server#resourceMesh} masked with 0x0000FFFF is used by many surface tiles to remember different kinds
     * of data. Farm tiles it's a bonus yeild related value, tree/bush tiles its damage, minedoor it's damage, dirt it's
     * a dig count at 0x000000FF, potion transmute in process power for various tiles at 0x0000FF00.
     * When a tile is converted to a different type it's a good idea to void out the resourceMesh entry.
     *
     * @param tilePos WU {@link TilePos} object. The tile to be altered.
     */
    static void voidWorldResourceEntry(TilePos tilePos){
        Server.setWorldResource(tilePos.x, tilePos.y, 0);
    }

    static byte getDiagonalRoadDir(Creature performer, int tilePosX, int tilePosY) {
        byte toReturn = 0;
        double angle = Math.toDegrees(Math.atan2(tilePosY - performer.getPosY(), tilePosX - performer.getPosX()));
        if(angle < 0){
            angle += 360;
        }
        boolean performerIsNwRelative = angle < 90;
        boolean performerIsNeRelative = angle >= 90 && angle < 180;
        boolean performerIsSeRelative = angle >= 180 && angle < 240;
        boolean performerIsSwRelative = angle >= 240;
        //TilePos performerTilePos = getPerformerNearestTile(performer);
        //int xSign = (int)Math.signum(performerTilePos.x - targetTilePos.x);
        //int ySign = (int)Math.signum(performerTilePos.y - targetTilePos.y);
        //boolean performerIsNwRelative = (xSign == 0 && ySign == 0) || (xSign == -1 && ySign == 0) || (xSign == -1 && ySign == -1) || (xSign == 0 && ySign == -1);
        if (performerIsNwRelative)
            toReturn = Tiles.TileRoadDirection.DIR_NW.getCode();
        if (performerIsNeRelative)
            toReturn = Tiles.TileRoadDirection.DIR_NE.getCode();
        if (performerIsSeRelative)
            toReturn = Tiles.TileRoadDirection.DIR_SE.getCode();
        if (performerIsSwRelative)
            toReturn = Tiles.TileRoadDirection.DIR_SW.getCode();
        return toReturn;
    }
}
